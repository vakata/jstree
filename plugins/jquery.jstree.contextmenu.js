(function ($) {
	$.extend($.tree.plugins, {
		"contextmenu" : {
			...
			defaults : {
				class_name : "hover",
				items_callback : function (n, items) { return items; },
				items : {
					create : {
						label	: "Create", 
						icon	: "create",
			...
			show : function(obj, t) {
				...
				var str = "";
				var cnt = 0;
				opts.items = opts.items_callback.call(null, obj, opts.items);
				for(var i in opts.items) {
					...
					if(opts.items[i].icon) str += "<ins " + (opts.items[i].icon.indexOf("/") == -1 ? " class='" + opts.items[i].icon + "' " : " style='background-image:url(\"" + opts.items[i].icon + "\"); background-position:0 0;' " ) + ">&nbsp;</ins>";
					else str += "<ins>&nbsp;</ins>";
				...
				$.tree.plugins.contextmenu.object.css({ "left" : (x), "top" : (y) }).stop(false,true).fadeIn("fast");
			},
			hide : function () {
				...
				$.tree.plugins.contextmenu.object.stop(false,true).fadeOut("fast");
			},
			exec : function (cmd) {
